...
fn test_library_component_props_schema_mixed_required_and_desc() {
    ...
    let comp = LibraryComponent {
        ...
        props_schema: Some(vec![
            PropSchema { name: "foo".to_string(), prop_type: "string".to_string(), required: true, description: Some("desc foo".to_string()) },
            PropSchema { name: "bar".to_string(), prop_type: "number".to_string(), required: false, description: None },
            PropSchema { name: "baz".to_string(), prop_type: "bool".to_string(), required: true, description: None },
        ]),
...
fn test_library_component_serialization_roundtrip() {
    ...
    let original = LibraryComponent {
        ...
        props_schema: Some(vec![
            PropSchema { name: "foo".to_string(), prop_type: "string".to_string(), required: true, description: Some("desc".to_string()) },
        ]),
        description: Some("desc".to_string()),
    };
...
fn test_library_component_props_schema_required_and_desc_some() {
    ...
    let comp = LibraryComponent {
        ...
        props_schema: Some(vec![
            PropSchema { name: "foo".to_string(), prop_type: "string".to_string(), required: true, description: Some("desc foo".to_string()) },
            PropSchema { name: "bar".to_string(), prop_type: "number".to_string(), required: true, description: Some("desc bar".to_string()) },
        ]),
        description: None,
    };
    let schema = comp.props_schema.as_ref().unwrap();
...
fn test_library_component_props_schema_required_and_desc_none() {
    ...
    let comp = LibraryComponent {
        ...
        category: "Basic".to_string(),
        props_schema: Some(vec![
            PropSchema { name: "foo".to_string(), prop_type: "string".to_string(), required: true, description: None },
            PropSchema { name: "bar".to_string(), prop_type: "number".to_string(), required: false, description: None },
        ]),
        description: None,
    };
...
fn test_library_component_props_schema_types() {
    ...
    let comp = LibraryComponent {
        ...
        category: "Basic".to_string(),
        props_schema: Some(vec![
            PropSchema { name: "placeholder".to_string(), prop_type: "string".to_string(), required: true, description: None },
            PropSchema { name: "max_length".to_string(), prop_type: "number".to_string(), required: false, description: None },
        ]),
        description: None,
    };
    let schema = comp.props_schema.as_ref().unwrap();
...
fn test_export_large_mixed_layout() {
    ...
    for i in 0..30 {
        ...
        layout.push(CanvasComponent::Text { content: format!("Text{}", i) });
        layout.push(CanvasComponent::Input { placeholder: format!("Ph{}", i) });
        layout.push(CanvasComponent::Container { children: vec![
            CanvasComponent::Button { label: format!("InnerBtn{}", i) },
...
fn test_canvas_component_container_with_mixed_children() {
    let comp = CanvasComponent::Container { children: vec![
        ...
        CanvasComponent::Text { content: "C".to_string() },
    ]};
...
fn test_export_deeply_nested_10_levels() {
    let mut comp = CanvasComponent::Button { label: "Deepest".to_string() };
    for _ in 0..10 {
        comp = CanvasComponent::Container { children: vec![comp] };
    }
    let layout = vec![comp];
...
fn test_canvas_component_button_empty_label() {
    let comp = CanvasComponent::Button { label: String::new() };
...
fn test_canvas_component_custom_empty_name() {
    let comp = CanvasComponent::Custom { name: String::new() };
...
fn test_export_mixed_components() {
    let layout = vec![
        CanvasComponent::Button { label: "Btn".to_string() },
...
fn test_export_deeply_nested() {
    let layout = vec![
        CanvasComponent::Container { children: vec![
            CanvasComponent::Container { children: vec![
                CanvasComponent::Button { label: "Deep".to_string() },
            ]},
        ]},
    ];
    let code = generate_leptos_code(&layout, &[]);
...
fn test_export_nested_custom_container_input() {
    ...
    let layout = vec![
        CanvasComponent::Container { children: vec![
            ...
            CanvasComponent::Input { placeholder: "Z".to_string() },
        ]},
    ];
    let custom_components = vec![
        LibraryComponent {
...
fn test_export_deeply_nested_custom_and_basic() {
    ...
    let layout = vec![
        CanvasComponent::Container { children: vec![
            ...
            CanvasComponent::Container { children: vec![
                CanvasComponent::Button { label: "B1".to_string() },
                CanvasComponent::Input { placeholder: "P1".to_string() },
            ]},
        ]},
    ];
    let custom_components = vec![
...
fn test_library_component_props_schema() {
    ...
    let comp = LibraryComponent {
        ...
        category: "Basic".to_string(),
        props_schema: Some(vec![
            PropSchema { name: "placeholder".to_string(), prop_type: "string".to_string(), required: false, description: Some("desc".to_string()) },
        ]),
        description: Some("desc".to_string()),
    };
    assert_eq!(comp.props_schema.as_ref().unwrap().len(), 1);
    assert_eq!(comp.props_schema.as_ref().unwrap()[0].name, "placeholder");
    assert_eq!(comp.props_schema.as_ref().unwrap()[0].description.as_deref(), Some("desc"));
}
...
fn test_canvas_component_equality() {
    let a = CanvasComponent::Button { label: "X".to_string() };
    let b = CanvasComponent::Button { label: "X".to_string() };
    let c = CanvasComponent::Button { label: "Y".to_string() };
...
fn test_canvas_component_input_no_placeholder() {
    let comp = CanvasComponent::Input { placeholder: String::new() };
...
fn test_canvas_component_text_empty() {
    let comp = CanvasComponent::Text { content: String::new() };
